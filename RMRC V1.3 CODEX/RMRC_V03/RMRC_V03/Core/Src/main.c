...
volatile uint32_t hundred_millis = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    if (htim->Instance == TIM1){
        hundred_millis += 1;
				//HAL_GPIO_TogglePin(jack_1_zero_point_green_led_GPIO_Port, jack_1_zero_point_green_led_Pin);
    }
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	while(AT24_ReadAll_IT() == HAL_BUSY){
		HAL_Delay(100);
	}
	while((AT24_State == 1) || (AT24_State == 3)){
		HAL_Delay(100);
	}
	if(AT24_State == 4){
		/*
        ...
        tx_buf[(f*2)+1] =  AT24_Data.vars[f]       & 0xFF;
			...
			while(AT24_page_write_counter != 1){HAL_Delay(10);}
...
void HAL_GPIO_EXTI_Rising_Callback(uint16_t GPIO_Pin)
{
		...
		switch(GPIO_Pin){
			...
			//	interfaceCallback(GPIO_Pin);
			//break;interface_button_Pin
			//case GPIO_PIN_12:
			//	interfaceCallback(GPIO_Pin);
			//break;
			default:
